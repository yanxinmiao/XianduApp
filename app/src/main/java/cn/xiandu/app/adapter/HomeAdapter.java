...
public class HomeAdapter extends BaseQuickAdapter<HomeBean> {
    ...
    public HomeAdapter(Context context ,int layoutResId, List<HomeBean> data) {
        super(layoutResId, data);
        this.context = context ;
    }

    @Override
    protected void convert(BaseViewHolder baseViewHolder, final HomeBean homeData) {
        baseViewHolder.setText(R.id.tv_desc, homeData.getSource())
...
//        baseViewHolder.setText(R.id.tv_time,homeData.getCtime());
        ...
        Glide.with(context)//如果this 是activity，当activity销毁时，Glide 将会自动停止请求。如果target 是独立于应用的 activity 生命周期，这时就要传application
                .load(homeData.getFirstImg())//source 可以是网络、资源文件、SD中图片文件和Uri,可以加载gif格式)
